...
void function () {
  ...
  extractor.title = function (html) {
    return title(cheerio.load(html));
  };
  extractor.favicon = function (html) {
    return favicon(cheerio.load(html));
  };
  extractor.description = function (html) {
    return description(cheerio.load(html));
  };
  extractor.keywords = function (html) {
    return keywords(cheerio.load(html));
  };
  extractor.lang = function (html) {
    return lang(cheerio.load(html));
  };
  extractor.canonicalLink = function (html) {
    return canonicalLink(cheerio.load(html));
  };
  extractor.tags = function (html) {
    return extractTags(cheerio.load(html));
  };
  extractor.image = function (html) {
    return image(cheerio.load(html));
  };
  extractor.videos = function (html, language) {
    var doc, lng, topNode;
    doc = cheerio.load(html);
    lng = language || lang(doc);
    cleaner(doc);
    topNode = calculateBestNode(doc, lng);
    return videos(doc, topNode);
  };
  extractor.text = function (html, language) {
    var doc, lng, topNode;
    doc = cheerio.load(html);
    lng = language || lang(doc);
    cleaner(doc);
    topNode = calculateBestNode(doc, lng);
    return text(doc, topNode, lng);
  };
