...
void function () {
  var _, addSiblings, attrPusher, baseValidation, biggestTitleChunk, cleanText, cleanTitle, definitions, formatter, generateCandidates, getFirstNonEmptyCandidate, getObjectTag, getScore, getSiblingsContent, getSiblingsScore, getVideoAttrs, isBoostable, isHighlinkDensity, isNodescoreThresholdMet, isTableAndNoParaExist, isValidAuthor, looksLikeDate, postCleanup, rawTitle, stopwords, textPusher, updateNodeCount, updateScore;
  _ = require('lodash');
  ...
  module.exports = {
    ...
    author: function (doc) {
      var authorCandidates, authorList, candidates, fallbackAuthor;
      authorList = [];
      authorCandidates = generateCandidates(doc, 'author');
      _.each(authorCandidates, function (candidate) {
        if (isValidAuthor(candidate))
          return authorList.push(cleanText(candidate));
      });
  ...
  getFirstNonEmptyCandidate = function (candidates, validationFn) {
    var bestCandidate, searching;
    searching = true;
    bestCandidate = null;
  ...
  baseValidation = function (candidate) {
    return candidate && candidate.trim();
  };
  looksLikeDate = function (candidate) {
    return baseValidation(candidate) && candidate.match(/\b[12]\d{3}\b/g);
  };
  generateCandidates = function (doc, key) {
    var queries;
    queries = [];
    _.each(definitions[key], function (selector) {
      return _.each(selector.elements, function (e) {
        var dom, query;
        query = e;
        ...
        if (!(selector.selectors || selector.attributes))
          textPusher(doc, queries, dom);
        _.each(selector.attributes, function (a) {
          if (!selector.selectors)
            return attrPusher(doc, queries, dom, a);
        });
        return _.each(selector.selectors, function (s) {
          return _.each(selector.filters, function (f) {
            query = e + '[' + s + "='" + f + "']";
            if (selector.select === 'first') {
              dom = doc(query).first();
            } else {
  ...
  textPusher = function (doc, list, dom) {
    return dom.each(function () {
      var cache$, cache$1, value;
      value = null != (cache$ = doc(this)) && null != (cache$1 = cache$.text()) ? cache$1.trim() : void 0;
      if (value)
        return list.push(value);
    });
  };
  attrPusher = function (doc, list, dom, attName) {
    return dom.each(function () {
      var cache$, cache$1, value;
      value = null != (cache$ = doc(this)) && null != (cache$1 = cache$.attr(attName)) ? cache$1.trim() : void 0;
      if (value)
        return list.push(value);
    });
  };
